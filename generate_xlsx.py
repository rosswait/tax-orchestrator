...
def create_tax_workbook(status="Single", dependents=0, year=2026):
    ...
    # --- 1. Tax Constants Tab ---
    ws_constants = wb.active
    ws_constants.title = "Tax Constants"
    
    ws_constants.append([f"Table A: Federal Brackets (Ordinary Income - {year})"])
    ws_constants.append(["Status", "Bracket Floor", "Base Tax", "Marginal Rate", "Standard Deduction"])
    fed_ord_brackets = [
    ...
    for row in fed_ord_brackets: ws_constants.append(row)
    ws_constants.append([])

    ws_constants.append([f"Table B: Federal Capital Gains Brackets ({year})"])
    ws_constants.append(["Status", "Bracket Floor", "LTCG Rate"])
    fed_cg_brackets = [
    ...
    for row in fed_cg_brackets: ws_constants.append(row)
    ws_constants.append([])

    ws_constants.append([f"Table C: California FTB Brackets ({year})"])
    ws_constants.append(["Status", "Bracket Floor", "Base Tax", "Marginal Rate", "Standard Deduction", "MH Surcharge Floor"])
    ca_brackets = [
    ...
    for row in ca_brackets: ws_constants.append(row)
    ws_constants.append([])

    ws_constants.append(["Table D: Surtaxes & Phaseouts"])
    ws_constants.append(["Status", "NIIT Threshold", "Addl Medicare", "CTC Phaseout Start"])
    surtaxes = [["Single", 200000, 200000, 200000], ["MFJ", 250000, 250000, 400000], ["MFS", 125000, 125000, 200000], ["HoH", 200000, 200000, 200000]]
    for row in surtaxes: ws_constants.append(row)

    # --- 2. Data Tabs ---
    ws_investments = wb.create_sheet("Investment Income Snapshots")
    ws_investments.append(["Quarter", "Entity", "Ordinary Dividends", "Qualified Dividends", "Short-Term Gains", "Long-Term Gains", "Tax-Exempt Interest"])
    ...
    ws_ds["A11"] = "Section 1.5: Projection & Assumptions"
    ...
    ws_ds["A17"] = "Section 2: Consolidated Income Projection"
    ...
    ws_ds["A21"] = "Investment Ordinary (STG + Div)"; ws_ds["B21"] = "=SUM('Investment Income Snapshots'!C:C) + SUM('Investment Income Snapshots'!E:E)"
    ws_ds["A22"] = "Investment Preferential (LTG + Qual)"; ws_ds["B22"] = "=SUM('Investment Income Snapshots'!D:D) + SUM('Investment Income Snapshots'!F:F)"
    ws_ds["A23"] = "Total Projected Federal AGI"; ws_ds["B23"] = "=B19 + B21 + B22"
    ws_ds["A24"] = "Total Projected CA AGI"; ws_ds["B24"] = "=B20 + B21 + B22"

    ws_ds["A26"] = "Section 3: Federal Tax Calculation"
    ...
    ws_ds["A32"] = "Section 4: CA Tax Calculation"
    ...
    ws_ds["A39"] = "Section 5: Final Liability & Surtaxes"
    ...
    ws_ds["D1"] = "Section 6: Estimated Tax Payments Ledger"
    ...
    ws_ds["A48"] = "Section 7: Payment Requirements"
    ...
    ws_ds["I4"] = "FED Target Milestone:"; ws_ds["J4"] = "=IFS(B9<=DATE(B8,4,15), B55, B9<=DATE(B8,6,15), B56, B9<=DATE(B8,9,15), B57, TRUE, B58)"
    ws_ds["I5"] = "FED Payments YTD:"; ws_ds["J5"] = "=B50"
    ws_ds["I6"] = "FED Cumulative Status:"; ws_ds["J6"] = "=IF(J5>=J4, \"✅ Met (Surplus: \" & TEXT(J5-J4, \"$#,##0\") & \")\", \"🔴 Shortfall (\" & TEXT(J4-J5, \"$#,##0\") & \")\")"
    ...
    ws_ds["I10"] = "CA Target Milestone:"; ws_ds["J10"] = "=IFS(B9<=DATE(B8,4,15), B61, B9<=DATE(B8,6,15), B62, B9<=DATE(B8,9,15), B63, TRUE, B64)"
    ws_ds["I11"] = "CA Payments YTD:"; ws_ds["J11"] = "=B52"
    ws_ds["I12"] = "CA Cumulative Status:"; ws_ds["J12"] = "=IF(J11>=J10, \"✅ Met (Surplus: \" & TEXT(J11-J10, \"$#,##0\") & \")\", \"🔴 Shortfall (\" & TEXT(J10-J11, \"$#,##0\") & \")\")"
    ...
    ws_ds["I19"] = "Effective CA Rate:"; ws_ds["J19"] = "=B37 / MAX(1, B23)" # Standardized to Federal AGI (B23)
    ...
    ws_ds["I28"] = "HSA CA Audit:"; ws_ds["J28"] = "=IF(B20=B19, \"OK\", \"CLEANUP: HSA DEDUCT\")"

    # --- Premium Formatting Engine ---
    style_sec = (Font(bold=True, size=11, color="FFFFFF"), PatternFill(start_color="2F75B5", end_color="2F75B5", fill_type="solid"))
    style_lbl = Font(bold=True); style_in = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid")
    sec_keys = ["Section ", "Table ", "SCHEDULE", "CENTER", "DIAGNOSTICS", "WARNINGS"]
    in_cells = ["B2", "B3", "B4", "B5", "B6", "B7", "B8", "B9", "B12", "B13", "B14", "B15"]

    for ws in wb.worksheets:
        for row in ws.iter_rows():
            for cell in row:
                val = str(cell.value)
                if any(k in val for k in sec_keys) or (ws.title != "Dashboard" and cell.row == 1): cell.font, cell.fill = style_sec
                elif (cell.column in [1, 4, 9] and cell.row > 1) or (ws.title == "Dashboard" and cell.column == 4 and cell.row == 2): cell.font = style_lbl
                if ws.title == "Dashboard":
                    if cell.coordinate in in_cells or (cell.column in [4,5,6,7] and 3 <= cell.row <= 10): cell.fill = style_in
                    if cell.column == 2:
        ...
        for col in ws.columns:
            ...
            ws.column_dimensions[col[0].column_letter].width = min(l + 2, 30)

    wb.save("Tax_Orchestrator_Template.xlsx")
    print(f"Tax logic synced and generated successfully.")
